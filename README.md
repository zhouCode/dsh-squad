...
- **键盘和辅助技术可完整操作**：工作台约束并恢复焦点，支持 Esc 关闭、Tab 循环和方向键切换标签；状态不只依赖颜色，并尊重系统的减少动画设置。
- **适合手机查看与处理**：小屏幕上的委派、计划和归档采用“列表 → 单项详情 → 返回列表”的主从导航，避免把短列表和长详情挤在同一屏；桌面端继续使用双栏布局。
...
承载 Relay 的节点会在`智能体收件箱 → 设置 → 连接诊断`显示仅限本机访问的运维概览，包括启动时间、已登记节点、实时邮箱连接、持久积压与最早待取时间、组织和邀请数量，以及容量与速率限制。这里仅汇总 Relay 自身状态，不读取任务正文，也不收集其他节点的私有运行指标。
...
更新页会在提交安装请求前列出完整的就绪检查：外部更新器是否配置、是否存在已验证的新版本、是否已有待处理请求、是否仍有处于分析/执行阶段的委派，以及是否有计划正在分派。任一条件未通过时，安装按钮会保持禁用并显示具体原因；外部更新器在真正停服前会使用同一套活动工作规则再次检查。
