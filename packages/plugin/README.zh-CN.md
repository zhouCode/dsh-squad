...
- 可审核的团队 Skill 目录：原生 Skill 经节点签名后发布，Owner/Admin 审核，成员显式安装后与原生 Skill 一起出现在 DSH `/` 菜单。
...
0.7.3 已在 DeepSeek Harness `0.1.1-rc.2` 上完成全部验收，并通过独立兼容回归继续支持 `0.1.0-rc.6`。
...
`智能体收件箱 → 团队 Skill`可以把 DSH 已发现的原生 Skill 作为签名、不可变版本发布到 Relay 组织目录。Member 的版本须由 Owner/Admin 审核，其他成员看到已批准版本后仍要在本机显式安装；加入组织不会自动安装任何 Skill。

安装项直接接入 DSH 原生 Skill Registry，所以原生 Skill 与团队 Skill 共用对话框中的 `/` 菜单和调用方式。界面提供`禁用`、`仅手动`、`本地`和`委派`四档开关；默认`仅手动`，只有`委派`允许 Agent 在 Squad 委派执行中自动选择该 Skill。撤销版本会自动禁用本机副本。

发布包只包含 `SKILL.md` 和目录资源，不授予工具、MCP、账号或凭据权限。发布和安装两端校验发布者签名、内容哈希、路径与体积限制；Relay 可读取 Skill 内容，Owner/Admin 审核仍是必要的代码信任边界。
...
只有明确的任务数据、公开状态、摘要、结果以及用户主动发布的团队 Skill 内容会跨 Node 传输。接收方的 Session ID、HumanTodo 详情、人工回复、凭据和工作区路径始终留在本地。
