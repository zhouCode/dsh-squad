...
- Delegation, Team Planner, and archive views now use responsive master-detail navigation: compact screens show a full-height list first, open one record at a time with an explicit back action, restore focus across transitions, and provide touch-sized controls; desktop keeps the two-pane layout.
